...
public interface CollegeRepo extends JpaRepository<College, Integer> {
    ...
    @Query(value = "SELECT COUNT(*) FROM tb_college c WHERE LOWER(c.status) = 'approved'", nativeQuery = true)
    ...
    @Query(value = """
            SELECT COUNT(*)
            FROM tb_college c
            INNER JOIN tb_college_type t ON c.college_type = t.id
            WHERE LOWER(t.name) = 'engineering'
